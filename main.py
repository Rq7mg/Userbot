...
PRE_USERS = {}        # user_id: premium kullanıcılar
...
# ---------------- PREMIUM ----------------
def is_premium(uid):
    return uid == OWNER_ID or uid in PRE_USERS
...
# ---------------- ETIKETLEME ----------------
GOOD_MORNING_MESSAGES = ["Günaydın ☀️"]
...
async def tag_all(uid, chat_id, text=None, type_msg=None):
    ...
    if not client:
        return await context.bot.send_message(chat_id, "⛔ Hesap bağlı değil.")
    await client.start()
    try:
